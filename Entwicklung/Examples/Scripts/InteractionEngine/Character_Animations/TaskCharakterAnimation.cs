...
public class TaskCharakterAnimation : MonoBehaviour
{
    ...
    private float talkingZoneDistance_used;
    private float radiusAroundObject_used;
    ...
    void Update()
    {
        ...
        /*
        if( UIconfig.interactingRangeMode == InteractingRangeMode.fromObserverView)
        {
            talkingZoneDistance_used = cursorMaxRange_fromObeserverView;
        }
        else
        {
            talkingZoneDistance_used = talkingZoneDistance;
        }
        */

        talkingZoneDistance_used = talkingZoneDistance;
        //If Player is in TalkingZone: TaskCharacter should look to the Player, stop walking and enable enter-key for talking
        if (Physics.Raycast(ray, out _, talkingZoneDistance_used, layerMask) || UIconfig.interactingRangeMode == InteractingRangeMode.fromObserverView)
        {
            this.walking = false;
            this.standing = true;
            this.timer = 0;
